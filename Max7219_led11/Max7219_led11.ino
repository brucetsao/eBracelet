...
//  Serial.println(")");
  ...
  showchar(&(alphabets[aa][0]),chipnum);
}
void showchar(byte *chr,int chipnum)
{
 int i = 0 ;
   //clearchar();
   lc.clearDisplay(chipnum);
 for(i = 0 ; i <5 ; i++)
 {
   lc.setColumn(chipnum,i,*(chr+i));
//   Serial.print(*(chr+i),HEX);
 } 
  // Serial.println("/");
 
}
void listallalpha(int delaytime,int chipnum)
{
    byte ii = 0 ;
  for(ii = 0 ; ii <=96; ii++)
  {
    showchar(&(alphabets[ii][0]),chipnum);
    delay(1000);
  }
  
 delay(delaytime);
}
void clearchar(int chipnum)
{
 int i = 0 ;
 for(i = 0 ; i <8 ; i++)
 {
   lc.setColumn(chipnum,i,0x00);
 } 
